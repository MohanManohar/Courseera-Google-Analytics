...
#epa_tuples=[tuple(state_list)]
#epa_tuples.append(tuple(county_list))
#epa_tuples.append(tuple(aqi_list))
...
#for i in range(0,len(epa_tuples[0])):
    #if epa_tuples[0][i] in aqi_dict:
        #aqi_dict[epa_tuples[0][i]].append((epa_tuples[1][i],epa_tuples[2][i]))
    #else:
        #aqi_dict[epa_tuples[0][i]] = [(epa_tuples[1][i],epa_tuples[2][i])]
...
#Average = [aqi_dict['California'][i][1] for i in range(0,len(aqi_dict['California']))]
...
def county_counter(state):
    ...
    for county,AQI in aqi_dict[state]:
        if county in county_count_dict:
            county_count_dict[county] += 1
        else:
            county_count_dict[county] = 1
    return county_count_dict
...
all_counties = []
for keys in aqi_dict:
    all_counties += list(county_counter(keys).keys())
